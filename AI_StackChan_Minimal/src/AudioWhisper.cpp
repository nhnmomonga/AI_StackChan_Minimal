...
// constexpr size_t record_number = 220;
...
void AudioWhisper::Record() {
  Serial.println("AudioWhisper::Record() - Starting...");
  Serial.flush();
...
#ifdef USE_ATOMIC_ECHO_BASE
  // マイクの設定を明示的に行う (M5Stack公式SDKより)
  // ES8311 codec を使用する Atomic Echo Base 専用設定
  ...
  Serial.printf("Mic config -> SR:%d, BCK:%d, WS:%d, DIN:%d, MCLK:%d, I2S:%d\n",
                ...
                mic_cfg.pin_mck,
                mic_cfg.i2s_port);
  Serial.println("Starting mic with explicit pin config (ES8311)...");
#else
  // Atomic Echo Base を使用しない場合は、プラットフォームのデフォルト設定を使用
  Serial.println("Starting mic with platform defaults...");
#endif
  Serial.flush();
